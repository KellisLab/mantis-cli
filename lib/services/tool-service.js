...
// Synthetic tool entry for `install_extension`. Extension installation is a
// developer REST endpoint rather than an MCP tool, so the CLI exposes it in the
// same local-tool layer as export and projection.
const LOCAL_INSTALL_EXTENSION_TOOL = {
  name: 'install_extension',
  description:
    'Install a local .mantisx, .zip, or JSON extension package for the API-key '
    + 'user in a specific Mantis space. The API-key user must own the space. '
    + 'This is a personal installation in that space, not a space-wide publish.',
  inputSchema: {
    type: 'object',
    properties: {
      file: {
        type: 'string',
        description: 'Path to the local extension package.',
      },
      space_id: {
        type: 'string',
        description: 'Target space UUID or Mantis space link.',
      },
    },
...
export class ToolService {
  constructor({ mcp, exporter, projection, extensionUpload }) {
    this.mcp = mcp;
    this.exporter = exporter;
    this.projection = projection;
    this.extensionUpload = extensionUpload;
  }
  ...
  async listTools() {
    ...
    tools.push(LOCAL_INSTALL_EXTENSION_TOOL);
    return { tools };
  }
  ...
  useTool(name, args = {}) {
    ...
    if (name === 'install_extension') {
      return this.extensionUpload.install(args.file, {
        spaceId: args.space_id ?? args.spaceId,
      });
    }
