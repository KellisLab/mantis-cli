...
export function installClaudePlugin() {
  ...
  const marketplaceFile = mergeMarketplaceIntoSettings(pluginRoot);

  try {
    runClaude(['plugin', 'marketplace', 'add', marketplaceFile, '--scope', 'user']);
    try {
    ...
    return { ok: true, method: 'cli', pluginRoot, marketplaceFile };
  } catch {
    return {
      ok: true,
      method: 'settings',
      pluginRoot,
      marketplaceFile,
      hint: 'Restart Claude Code, open /plugin, then /reload-plugins',
    };
  }
}
