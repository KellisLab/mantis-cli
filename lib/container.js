...
import { ExtensionUploadService } from './services/extension-upload-service.js';
...
export function createContainer(overrides = {}) {
  ...
  const extensionUpload = new ExtensionUploadService({ configStore, client });
  const tools = new ToolService({ mcp, exporter, projection, extensionUpload });
