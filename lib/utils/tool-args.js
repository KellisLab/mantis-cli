...
/**
 * May we read stdin (fd 0) without risking an interactive hang? We read it only
 * when it is NOT a terminal. Reading a TTY blocks forever waiting for EOF, and
 * our main caller is non-interactive agents, so a silent hang is the worst
 * outcome — `isTTY` is exactly what rules that case out.
 *
 * We intentionally do NOT try to fstat the stream to distinguish a pipe from
 * /dev/null: on Windows a real pipe carrying data reports neither isFIFO() nor
 * isFile() (only the raw mode bits hint at it), so fstat-based detection gives
 * false negatives there. Instead we read whenever stdin isn't a TTY and let the
 * caller treat an empty read as "no args" — reading /dev/null or a closed handle
 * returns empty immediately on every platform, so this never hangs in practice.
 */
...
export function parseToolArgs(argv) {
  const args = {};
  let sawExplicitArgs = false; // --args / --args-file / --args-stdin given?
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--args-stdin') {
      sawExplicitArgs = true;
      if (!stdinReadable()) {
        throw new Error(
          '--args-stdin expects JSON piped on stdin, but stdin is a terminal. ' +
          "Pipe it in (e.g. `echo '{\"uris\":[...]}' | mantis use <tool> --args-stdin`), " +
          'or use --args-file <path>.',
        );
      }
      const raw = readStdin();
      if (clean(raw) === '') {
        throw new Error(
          '--args-stdin was given but stdin was empty. Pipe JSON in, or use --args-file <path>.',
        );
      }
  ...
  // Implicit stdin: `echo '{...}' | mantis use <tool>` with no args flag. Only
  // when NOTHING else was passed (no --args*, no flat flags) and stdin isn't a
  // terminal — so a flat-flag call is never second-guessed. Reading /dev/null or
  // a closed handle returns empty, which we treat as "no args" ({}), so
  // `mantis use get_space_context` keeps working with no pipe.
  if (!sawExplicitArgs && Object.keys(args).length === 0 && stdinReadable()) {
    const raw = readStdin();
    if (clean(raw) !== '') Object.assign(args, parseArgsBlob(raw, 'stdin'));
  }
