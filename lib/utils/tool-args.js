...
export function parseToolArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--args-stdin') {
      // Guard against a hang: reading fd 0 when stdin is a terminal (nothing
      // piped) blocks forever waiting for EOF. Agents invoke us non-interactively
      // so a silent hang is worse than a loud error — bail with guidance instead.
      if (process.stdin.isTTY) {
        throw new Error(
          '--args-stdin expects JSON piped on stdin, but nothing is piped. ' +
