...
export function parseToolArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // parses it and cannot strip its quotes. The quote-proof cross-shell path.
    //
    // Reading stdin is gated behind THIS EXPLICIT FLAG — never implicitly on a
    // bare `mantis use <tool>`. An interactive terminal (e.g. Cursor's, VS
    // Code's) hands a child an open pipe with no data and no EOF, which is NOT a
    // TTY, so a `readFileSync(0)` triggered just because "stdin isn't a TTY"
    // blocks forever. Requiring the flag means a plain command never touches
    // stdin and never hangs; when the flag is present, the caller has committed
    // to piping, and the normal `echo … | mantis …` closes the writer at once.
    if (a === '--args-stdin') {
